...
<script type="text/javascript">
	function correction(mem_id) {
		window.open('MemberInf?mem_id=' + mem_id, '_parent ',
				'width=900px,height=700px,left=400, top=200');
	}
</script>
<body>
	<header>
					...
					<div align="right">
						<form action="memberList">
							...
							<select name="searchType">
								<option value="name"
									<c:if test="${param.searchType eq 'name'}">selected</c:if>>이름</option>
								<option value="idx"
									<c:if test="${param.searchType eq 'idx'}">selected</c:if>>회원번호</option>
						...
						<table border="1" class="graph">
							...
							<c:forEach var="member" items="${memberList}">
								<tr>
									...
									<th><c:set var="jumin" value="${member.mem_jumin}" />
										${fn:substring(jumin,0,6)}</th>
									...
									<th><input type="button" value="상세정보 보기"
										onclick="correction('${member.mem_id}')"></th>
								</tr>
							</c:forEach>
						</table>
					</form>
